...
const wrapDependencies = require("./wrapDependencies");
        ...
        plugins: [
            ["module-resolver", {
                ...
                resolvePath(sourcePath, currentFile, opts) {
                    return moduleToAlias(sourcePath, aliases).replace(/\.js$/, "");
                }
            }],
...
const getJS = async (file, aliases, cyclicDependencies) => {
    ...
    code = wrapDependencies(code, file, aliases, cyclicDependencies);
...
const getExternalModuleData = async (file, aliases, cyclicDependencies) => {
    ...
    return {
        code: await getJS(file, aliases, cyclicDependencies),
        mime: "application/javascript; charset=UTF-8"
    };
};

const generateAll = async (packageName, conf, aliases, cyclicDependencies) => {
    ...
    return Promise.all(input.map(async fileName => {
        ...
        return {
            fileName: relativeFileName,
            code: await getJS(fileName, aliases, cyclicDependencies)
        };
    }));
};
