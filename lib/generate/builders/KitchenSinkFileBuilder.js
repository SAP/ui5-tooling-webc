...
class KitchenSinkFileBuilder extends BuilderBase {
	constructor(config, library, allEntities) {
		...
		this.controlsWithTestPages = this.libraryEntities.filter(control => !!control.tagname && !this.config?.noControl.includes(control.name) && !this.config?.noTestAssets.includes(control.name));
