...
const calculateAPI = (component, entries, calculated) => {
    ...
    entities.forEach(entity => {
        component[entity] = removeEmpty(component[entity] || []).filter(x => x.visibility === "public" && !x.deprecated);
    });
...
const getAllEntities = (apiFiles, replaceJSDocNamespaces) => {
    ...
    apiFiles.forEach(fileText => {
        ...
        mergedAPIFile = merge(mergedAPIFile, api);
    });

    return getAPI(mergedAPIFile.symbols);
}
