...
class KitchenSinkTemplateBuilder extends TemplateBuilderBase {
	constructor(config, library, allEntities) {
		super(config, library, allEntities);

		this.controlsWithTestPages = this.libraryEntities.filter(control => this.config.entryPoints.includes(control.name));
...
module.exports = KitchenSinkTemplateBuilder;
