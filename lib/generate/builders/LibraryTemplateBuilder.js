const TemplateBuilderBase = require("./TemplateBuilderBase.js");
const getVersion = require("../utils/getVersion.js");
const beautifyJS = require("../utils/beautifyJS.js");

class LibraryTemplateBuilder extends TemplateBuilderBase {
	constructor(config, library, allEntities, packagesForThisLibrary) {
		super(config, library, allEntities);
        this.packagesForThisLibrary = packagesForThisLibrary;
	}
	...
	generate() {
		const controls = this.libraryEntities.filter(entry => !!entry.tagname && !this.config.excludeList.includes(entry.name));
		const types = this.libraryEntities.filter(entry => !entry.abstract && !entry.tagname && !this.config.excludeList.includes(entry.name) && entry.kind !== "interface");
		...
		template += `
		CSP.setPackageCSSRoot("${this.packagesForThisLibrary[0]}", sap.ui.require.toUrl("${this.library.split(".").join("/")}/thirdparty/css/"));
...
module.exports = LibraryTemplateBuilder;
