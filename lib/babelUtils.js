...
const processWithBabel = async (code, aliases) => {
    ...
    const output = await babel.transformAsync(code, {
        ...
        plugins: [
            [path.join(__dirname, "../node_modules/babel-plugin-module-resolver/lib/index.js"), {
                extensions: [".js"],
                resolvePath(sourcePath, currentFile, opts) {
                    return resolve(sourcePath).replace(/\.js$/, "");
                }
            }],
            path.join(__dirname, "../node_modules/babel-plugin-transform-dynamic-import/lib/index.js"),
            path.join(__dirname, "../node_modules/@babel/plugin-transform-modules-amd/lib/index.js")
        ]
    });
...
const generateAll = async (conf, aliases) => {
    let input = await glob(`${conf.inputPath}/**/*{.js,.json,.css}`);

    if (conf.include) {
        input = input.filter(file => conf.include.some(allowedName => file.includes(allowedName)));
    }
    if (conf.exclude) {
        input = input.filter(file => !conf.exclude.some(deniedName => file.includes(deniedName)));
    }

    return Promise.all(input.map(async fileName => {
        const relativeFileName = fileName.substr(conf.inputPath.length);
