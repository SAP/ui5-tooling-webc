const normalizeLibraryName = require("./normalizeLibraryName.js");
const normalizeType = require("./normalizeType.js");
const process = require("process");
const registerType = require("./debug.js").registerType;
...
class ControlModel {
	...
	generateInterfaces() {
		if (this.control.implements) {
			this.interfaces = this.control.implements.map(int => normalizeLibraryName(int));
		}
	...
	buildPropertyType(propInfo) {
		...
		if (typeInfo) {
			type = normalizeLibraryName(typeInfo.name);
		}
	...
	generateEnums() {
		[...this.neededEnums.values()].forEach(e => {
			...
			}else if (lib === "sap.ui.core") {
				this.extraLibraries.add("core");
				libraryVarName = "coreLibrary"
			} else if (lib === "sap.ui.webc.main") {
				libraryVarName = "mainLibrary";
				this.extraLibraries.add("main");
			} else {
				console.log("Unknown library:", lib);
				process.exit();
			}
	...
	generateAggregations() {
		...
		slots.forEach((slot) => {
			...
			if (slot.type.startsWith("Node") || slot.type.startsWith("HTMLElement")) {
				...
				type = normalizeLibraryName(slot.type.split("[]")[0]);
			}
	...
	hasAssociationLikeDescription(description) {
		return description.toLowerCase().includes("id of");
	}
	...
	hasDesignTime() {
		return (this.config.designtime || {}).hasOwnProperty(this.fullName);
	}
}
