...
class ControlFileBuilder {
	constructor(control, existingContent = "") {
	...
	generate() {
		...
		sap.ui.define([
			 ...
			 */
			var ${this.control.basename} = WebComponent.extend("${this.control.name}", {
				metadata: {
					library: "${this.control.library}",
					tag: "${this.control.tagWithSuffix}"
...
: "";
	...
	get designTimeString() {
		...
		const libraryPath = this.control.library.split(".").join("/");

		return `,
		designtime: "${libraryPath}/designtime/${this.control.name}.designtime"`;
	}
