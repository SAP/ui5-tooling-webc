...
const ControlTemplateBuilder = require("./ControlTemplateBuilder.js");
const TestPageTemplateBuilder = require("./TestPageTemplateBuilder.js");
const KitchenSinkTemplateBuilder = require("./KitchenSinkTemplateBuilder.js");
const LibraryTemplateBuilder = require("./LibraryTemplateBuilder.js");
...
const generate = async (library, config, apiPaths, packagesForThisLibrary) => {
    ...
    const generateLibrary = allEntities => {
        ...
        let librarySource = builder.generate();
        librarySource = postFormat(librarySource);
...
`);
    ...
    const generateControl = (entry, allEntities, index) => {
        ...
        let controlSource = builder.generate();
        controlSource = postFormat(controlSource);


        fs.writeFileSync(outputFileName, controlSource);
    };

    const generateTestPageQUnitAndDemokitSample = (entry, allEntities, index) => {
        const name = entry.basename;
...
// build the API
    let allEntities = [];
    for (let i in apiPaths) {
        const apiFile = apiPaths[i];
...
// generate all test pages and qunits
    entryPointControls.forEach((entry, index) => {
        generateTestPageQUnitAndDemokitSample(entry, allEntities, index);
    });
