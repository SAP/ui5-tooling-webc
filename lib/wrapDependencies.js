 ...
 * @returns {void|string|*}
 */
const wrapDependencies = (moduleCode, moduleName, aliases) => {
    // Get the absolute path of the module
    if (!moduleName.includes("node_modules/")) {
        return moduleCode;
    }
    const nodeModulesPath = moduleName.split("node_modules/")[1];
    const resolvedModulePath = resolve(nodeModulesPath, aliases);
    ...
    moduleCode = estraverse.replace(ast, {
        enter: (node, parent) => {
            if (node.type === "Identifier") {
                ...
                if (deps.includes(node.name) && !node.__replaced && node.range[0] > depsLastIndex) {
                    ...
                    const newStr = `(${node.name} || (${node.name} = sap.ui.require("${moduleName}")))`;
                    const newCode = espree.parse(newStr, { ecmaVersion: "latest" });
                    newCode.body[0].expression.left.__replaced = true;
                    newCode.body[0].expression.right.left.__replaced = true;
