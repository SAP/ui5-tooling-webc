const normalizeType = require("../../generate/utils/normalizeType");
...
class ComponentsPackage {
    ...
    createControl(declaration) {
        ...
        const control = {
            ...
            tagWithSuffix: `${declaration.tagName}${this.constructor.config.suffix || ""}`,
    ...
    getEventsForDeclaration(declaration) {
        const fullControlName = `${this.namespace}.${declaration.name}`;
        const events = (declaration.events || []).filter(event => event._ui5privacy === "public").map(event => {
            return {
    ...
    getMethodsForDeclaration(declaration) {
        const methods = (declaration.members || []).filter(member => member.kind === "method").map(method => {
            return {
                name: method.name,
                description: method.description,
                "return": {
                    type: method.return.type.text,
                    description: method.return.description
                },
    ...
    getGettersForDeclaration(declaration) {
        const getters = (declaration.members || []).filter(member => member.kind === "field" && member.readonly).map(getter => {
            const name = "get" + getter.name.substr(0, 1).toUpperCase() + getter.name.substr(1);
            let description;
