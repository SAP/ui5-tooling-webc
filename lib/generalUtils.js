...
const getReverseAliases = (customConfigurations, includeAllLibraries = false) => {
    let reverseAliases = {};

    customConfigurations.forEach(customConfig => {
        if (customConfig && customConfig["ui5-tooling-webc"]) {
            const includeLibrary = includeAllLibraries || !customConfig["ui5-tooling-webc"].isStockLibrary;
