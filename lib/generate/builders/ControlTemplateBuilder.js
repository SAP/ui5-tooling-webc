...
quoteReservedWord = str => {
	return reserved.check(str, 6) ? `"${str}"` : str;
};

class ControlTemplateBuilder extends TemplateBuilderBase {
	...
	generate() {
		...
		// Extra libraries
		if (this.model.extraLibraries.has("core")) {
			requiredModules.push(`"sap/ui/core/library"`);
			requiredModulesNames.push(`coreLibrary`);
		}
		if (this.model.extraLibraries.has("main")) {
			requiredModules.push(`"sap/ui/webc/main/library"`);
			requiredModulesNames.push(`mainLibrary`);
		}
...
module.exports = ControlTemplateBuilder;
