const { getReverseAliases, getAliases, getCyclicDependencies, getExtraResolvePaths } = require("./generalUtils");
...
module.exports = async function serveWebComponents({ log, middlewareUtil, options, resources }) {
    ...
    const extraResolvePaths = await getExtraResolvePaths();
    ...
    return async (req, res, next) => {
        ...
        const externalFile = await getExternalFile(req.path, reverseAliases, extraResolvePaths);

        // If yes, find it in node_modules/, transpile it, and serve it.
        if (externalFile) {
            const { code, mime } = await getExternalModuleData(externalFile, aliases, cyclicDependencies);
